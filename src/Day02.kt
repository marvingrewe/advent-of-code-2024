...
fun main() {
    ...
    fun List<Int>.isSafeReport(): Boolean = this.zipWithNext().map { it.first - it.second }
        .run { this.all { it in 1..3 } || this.all { it in -3..-1 } }

    fun part1(input: List<String>): Int {
        return input.toIntLists().count { it.isSafeReport() }
    }

    fun List<Int>.isSafeReportDampened(): Boolean =
        this.isSafeReport() || indices.map { this.filterIndexed { index, _ -> index != it } }
            .any { it.isSafeReport() }

    fun part2(input: List<String>): Int {
        return input.toIntLists().count { it.isSafeReportDampened() }
    }
